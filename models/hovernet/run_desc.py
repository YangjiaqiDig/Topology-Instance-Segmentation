...
from .utils import crop_to_shape, dice_loss, mse_loss, msge_loss, xentropy_loss, topo_loss
import torchvision
from collections import OrderedDict
from topoloss_pytorch import getCriticalPoints, compute_dgm_force
...
def valid_step(batch_data, run_info):
    ...
    # HWC
    true_np_onehot = (F.one_hot(true_np.to("cuda").type(torch.int64), num_classes=2)).type(torch.float32)
    true_dict_loss = {
        "np": true_np_onehot,
        "hv": true_hv.to("cuda").type(torch.float32),
        "topo": true_np.to("cuda").type(torch.int64)
    }
    ...
    if model.module.nr_types is not None:
        ...
        true_dict_loss["tp"] = true_tp_onehot
    ...
    with torch.no_grad():  # dont compute gradient
        ...
        for branch_name in pred_dict.keys():
            for loss_name, loss_weight in loss_opts[branch_name].items():
                loss_func = loss_func_dict[loss_name]
                loss_args = [true_dict_loss[branch_name], pred_dict[branch_name]]
                if loss_name == "topo":
                    loss_args[0] = true_dict_loss["topo"]
                if loss_name == "msge":
                    loss_args.append(true_np_onehot[..., 1])
                term_loss = loss_func(*loss_args)
                track_value("loss_%s_%s" % (branch_name, loss_name), term_loss.cpu().item())
    ...
    result_dict["raw"] = {  # protocol for contents exchange within `raw`
        ...
        "prob_np": pred_dict["np"][..., 1].cpu().numpy(),
...
def infer_step(batch_data, model):
    ...
    with torch.no_grad():  # dont compute gradient
        ...
        lh_patch = torch.squeeze(pred_dict["np"].clone().cpu().detach()).numpy()
        pd_lh, bcp_lh, dcp_lh, pairs_lh_pa = getCriticalPoints(lh_patch[0])
        force_list, idx_holes_to_fix, idx_holes_to_remove = compute_dgm_force(pd_lh, pd_lh, pers_thresh=0.03)
        for i in range(pred_dict["np"].size(0)):
           torchvision.utils.save_image(pred_dict["np"][i].squeeze(-1), 'npImages/{}.png'.format(i))
        if "tp" in pred_dict:
