...
####
def topo_loss(true, pred):
    """Calculate topological loss. Assumes NHWC!
    ...
    """
    total_loss = 0
    pred_topo = pred[..., 1] # N * H * W
    for i in range(pred_topo.shape[0]):
        total_loss += getTopoLoss(pred_topo[i], true[i])
    
    return total_loss / pred_topo.shape[0]
