...
def run_nuclei_type_stat(pred_dir, true_dir, type_uid_list=None, exhaustive=True):
    ...
    for file_idx, filename in enumerate(file_list[:]):
        ...
        # print(np.unique(true_inst_type), np.unique(pred_inst_type))
    ...
    titles = ['f1_m', 'f1_i', 'f1_e', 'f1_s']
    results_title = {'f1_d': f1_d, 'classify_acc': acc_type}
    for type_uid in type_uid_list:
        ...
        results_title[titles[type_uid-1]] = f1_type

    np.set_printoptions(formatter={"float": "{: 0.5f}".format})
    for key, value in results_title.items():
        print(f'{key:20}{value:0.5f}')
