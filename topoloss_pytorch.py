...
def getCriticalPoints(likelihood):
    ...
    if len(pd_lh)==0: 
        return pd_lh, bcp_lh, dcp_lh, False
    return pd_lh, bcp_lh, dcp_lh, True

def getTopoLoss(likelihood_tensor, gt_tensor, topo_size=80):
    """
    ...
    Args:
        ...
        topo_size        :   The size of the patch is used. Default: 80
    Returns:
        loss_topo        :   The topology loss value (tensor)
    """
    torchvision.utils.save_image(likelihood_tensor, 'npImages/lh.png')
    torchvision.utils.save_image(gt_tensor.float(), 'npImages/gt.png')
    # Sigmoid if not activate before
    ...
    for y in range(0, likelihood.shape[0], topo_size):
        for x in range(0, likelihood.shape[1], topo_size):
            ...
            try:
                ...
                img = Image.fromarray(likelihood, 'L')
                img.save('errorlh.png')
                imggt = Image.fromarray(gt, 'L')
                imggt.save('errorgt.png')
                print(likelihood, pd_lh)
                sss
